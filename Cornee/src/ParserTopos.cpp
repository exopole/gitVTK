     ...
     while(index < (int)stringList.size())
        ...
        else
        {
            ...
            else if (stringList[index][0] == "Ref Center: X, Y, Z[mm]")
            {
                if (name == " Anterior Best Fit Sphere")
                {
                    coordBFSAnterior.setCoordonne(ParserString::StringtoFloatVector(stringList[index][1],delimiterList));
                    //UtilsVector::printVector(ParserString::StringtoFloatVector(stringList[index][1],delimiterList));
                }
                else
                    coordBFSPosterior.setCoordonne(ParserString::StringtoFloatVector(stringList[index][1], delimiterList));
            }

            else if (stringList[index][0] == "Y[mm]")
            {
                coordXY = ParserString::StringtoFloatVector(stringList[index][1],delimiterList);
...
/***
*\fn std::vector<float> ParserTopos::getCoordBFSAnterior()
...
/**
* \fn float getRadiusBFSAnterior()
* \brief  
*/
float ParserTopos::getRadiusBFSAnterior()
{
    return radiusBFSAnterior;
}
float ParserTopos::getRadiusBFSPosterior()
{
